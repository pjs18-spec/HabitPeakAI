...
struct DailySignalView: View {
    ...
    // MARK: - Navigation
    @State private var navigateToSignals = false
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                // Hidden NavigationLink for programmatic navigation to SignalsView
                NavigationLink(
                    destination: SignalsView().environment(\.modelContext, modelContext),
                    isActive: $navigateToSignals
                ) { EmptyView() }
                ...
                ScrollView(showsIndicators: false) {
                    ...
                    .padding(.bottom, bottomCTAHeight)
                }
            }
    ...
    private var insightSummarySection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 12) {
                NavigationLink(destination: SignalsView().environment(\.modelContext, modelContext)) {
                    summaryButton(title: "Signals", systemImage: "waveform.path.ecg")
                }
                
                NavigationLink(destination: HabitTrackerView().environment(\.modelContext, modelContext)) {
                    summaryButton(title: "Tracker", systemImage: "checkmark.circle")
                }
                
                NavigationLink(destination: HistoryTrendView()) {
                    summaryButton(title: "Trends", systemImage: "chart.line.uptrend.xyaxis")
                }
            }
        }
        .padding(16)
    ...
    private func generateInsights() {
        ...
        Task { @MainActor in
            ...
            // <-- Trigger navigation to SignalsView
            navigateToSignals = true
        }
    }
